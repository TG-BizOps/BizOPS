...
### GitHub
- **BizOPS 리포**: `git@github-personal:TG-BizOps/BizOPS.git` (branch: main, 포탈)
- **toomics-ai-agent 리포**: `github-personal:madgegja/toomics-ai-agent.git` (branch: main, 소스)
...
적용 대상 디렉토리:
- `/Claude/BizOPS/` → `TG-BizOps/BizOPS` 리포
- `/Claude/toomics-ai-agent/` → `madgegja/toomics-ai-agent` 리포

예외: 사용자가 "푸시하지 마", "커밋만" 등 명시적으로 제외 지시한 경우

### BizOPS 인덱스 자동 동기화 규칙
`/Claude/toomics-ai-agent/`에서 HTML 리포트를 생성·수정·삭제한 경우, toomics-ai-agent 커밋·푸시 후 추가로:
1. 해당 파일을 `/Claude/BizOPS/`로 복사 (루트 + prefix 기반 하위 폴더: cs_*→cs/, chargeback_*→chargeback/, faq_*→faq/)
2. `/Claude/BizOPS/index.html`에서 해당 파일의 버전 뱃지 + VERSION_HISTORY 업데이트
3. `bash scripts/update_changelog.sh` 실행하여 변경 히스토리 자동 갱신
4. 변경된 파일 + index.html을 커밋·푸시 (`TG-BizOps/BizOPS` 리포)
...
### 기본 원칙
- **싱글 파일**: `<style>` 태그에 모든 스타일 포함. Chart.js CDN은 주간 HTML 리포트에서 허용
- **차트**: 기본 CSS-only 바차트, 주간 리포트는 Chart.js CDN 사용 가능
- **라이트모드만**: 다크모드 구현 금지 (`prefers-color-scheme: dark`, `data-theme="dark"` 등 사용 금지)
...
/* 다크모드 금지 — @media (prefers-color-scheme: dark) 블록 사용하지 않음 */
```
